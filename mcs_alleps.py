...
def append_matching_data(source_file, target_file):
    ...
    target_data = {str(row[1]).lower().replace(" ", "").replace("_", "").replace("(", "").replace(")", ""): row for row in target_ws.iter_rows(values_only=True)}
    ...
    appended_filename = os.path.splitext(source_filename)[0] + "_AllEpsChecked.xlsx"
...
if __name__ == "__main__":
    ...
    source_file = "V:\\Projects\\Python Scripts\\Music Cue Sheet Script\\agh908 music edl_MusicCueSheet.xlsx"
    target_file = "V:\Projects\Python Scripts\Music Cue Sheet Script\AGH2-9_ALL EPs_Seamless_MCS.xlsx"
